...
class Room:
    def __init__(self):
        ...
        self.players: Dict[float, WebSocket] = {}  # player_id: websocket

ROOMS: Dict[str, Room] = {}

async def handle_main(ws: WebSocket, room_name: str):
...
async def handle_room(ws: WebSocket, ROOM: Room):
    # Add new player
    id = time.time()
    ROOM.players[id] = ws
    ...
    await ws.send_text(json.dumps({"event": "waiting", "players": len(ROOM.players)}))
    ...
    if len(ROOM.players) == 2 and not ROOM.game_started:
        ...
        await asyncio.gather(*(player_ws.send_text(start_msg) for player_ws in ROOM.players.values()))
...
async def handle_game_message(ws: WebSocket, player_id: float, data, ROOM: Room):
    if data["type"] == "click":
        ...
        if ROOM.game.board[x][y] == -1:
            ...
            await ws.send_text(json.dumps({"type": "reveal", "data": revealed, "found_count": len(ROOM.game.revealed[player_id])}))

            # Check for win
            if ROOM.game.check_win(player_id):
                await handle_win(player_id, ROOM)

async def handle_loss(player_id: float, ROOM: Room, finish_time=None):
    """Handle win/loss
    ...
    await loser_ws.send_text(json.dumps({"type": "end", "result": "lose", "time": finish_time}))
...
async def handle_win(player_id: float, ROOM: Room, complete=True):
    """Handle win
    ...
    if complete:
        ...
        await winner_ws.send_text(json.dumps({"type": "end", "result": "win", "time": finish_time}))
        for id in ROOM.players.keys():
            if id != player_id:
                await handle_loss(id, ROOM, finish_time)
    else:
        await winner_ws.send_text(json.dumps({"type": "end", "result": "win"}))